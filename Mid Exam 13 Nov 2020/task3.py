...
while not command == "Retire":
    ...
    if command_type == "Fire":
        ...
        if index in range(len(pirate_ship)) and health <= max_health_per_section:
            pirate_ship[index] += health
    elif command_type == "Status":
        to_repair = [el for el in range(len(pirate_ship)) if el < max_health_per_section]
        counter = to_repair.count()
